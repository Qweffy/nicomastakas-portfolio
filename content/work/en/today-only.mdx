...
It's the most honest kind of portfolio piece: a product I use every single day, where the value is entirely in the product judgement, not the line count. Native on two platforms (SwiftUI + SwiftData on macOS, my daily driver, plus a lightweight Tauri build on Windows), shipped, lived-in, and still iterating, with the macOS build getting the most maintenance since it's the one I use.

## Results

- **Daily driver, still iterating**: I use it every day and keep shipping changes to it; macOS is my main platform, so the SwiftUI build gets the most maintenance.
